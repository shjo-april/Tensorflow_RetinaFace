...
# 1. dataset
test_data_list = np.load('./dataset/validation_detection.npy', allow_pickle = True)
...
saver.restore(sess, './model/RetinaNet_{}.ckpt'.format(130000))

batch_image_data = np.zeros((BATCH_SIZE, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNEL), dtype = np.float32)

for i in range(len(test_data_list) // BATCH_SIZE):
    total_gt_bboxes = []
